...
import '../datasources/halaqa_remote_data_source.dart';
import 'halaqa_sync_service.dart';

/// The concrete implementation of the [HalaqaSyncService].
///
/// This class contains the core logic for the two-way, delta-based synchronization
/// for halaqa data, including handling paginated responses from the server.
@LazySingleton(as: HalaqaSyncService)
final class HalaqaSyncServiceImpl implements HalaqaSyncService {
  final HalaqaRemoteDataSource _remoteDataSource;
  final HalaqaLocalDataSource _localDataSource;
  final NetworkInfo _networkInfo;

  bool _isSyncing = false;

  HalaqaSyncServiceImpl({
    required HalaqaRemoteDataSource remoteDataSource,
  ...
  Future<void> performSync({int initialPage = 1}) async {
    ...
    _isSyncing = true;
    print('[SyncService] Starting two-way sync for halaqas...');
    try {
    ...
    for (final op in pendingOps) {
      try {
        if (op.operationType == 'upsert') {
          final serverModel = await _remoteDataSource.upsertHalaqa(
            op.decodedPayload!,
          );
          await _localDataSource.upsertHalaqa(serverModel);
        } else if (op.operationType == 'delete') {
          await _remoteDataSource.deleteHalaqa(op.entityUuid);
        }
  ...
  Future<void> _pullRemoteChanges({required int initialPage}) async {
    ...
    while (hasMorePages) {
      ...
      final syncResult = await _remoteDataSource.getUpdatedHalaqas(
      ...
      if (syncResult.updated.isNotEmpty || syncResult.deleted.isNotEmpty) {
        await _localDataSource.applySyncBatch(
          ...
          deletedHalaqas: syncResult.deleted,
        );
      }
