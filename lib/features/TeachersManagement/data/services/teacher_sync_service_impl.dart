...
import 'teacher_sync_service.dart';

/// The concrete implementation of the [TeacherSyncService].
///
/// This class contains the core logic for the two-way, delta-based synchronization
/// for teacher data, including handling paginated responses from the server.
@LazySingleton(as: TeacherSyncService)
final class TeacherSyncServiceImpl implements TeacherSyncService {
  ...
  Future<void> performSync({int initialPage = 1}) async {
    ...
    _isSyncing = true;
    print('[SyncService] Starting two-way sync for teachers...');
    try {
    ...
    for (final op in pendingOps) {
      try {
        if (op.operationType == 'upsert') {
          final serverModel = await _remoteDataSource.upsertTeacher(
            op.decodedPayload!,
          );
  ...
  Future<void> _pullRemoteChanges({required int initialPage}) async {
    ...
    while (hasMorePages) {
      ...
      final syncResult = await _remoteDataSource.getUpdatedTeachers(
      ...
      if (syncResult.updated.isNotEmpty || syncResult.deleted.isNotEmpty) {
        await _localDataSource.applySyncBatch(
          ...
          deletedTeachers: syncResult.deleted,
        );
      }
